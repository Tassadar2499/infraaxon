...
Agents refresh configuration and collect a heartbeat every thirty seconds. A direct diagnosis obtains one specialist assessment with linked context. An environment investigation collects fresh baseline observations from enabled components (concurrency four), then selects up to six specialists using the target, direct dependencies, context sources, reverse dependencies and Russian/English symptom keywords. Failed observations increase routing priority. Up to eight linked sources provide context through their own authenticated agents. Results are persisted after each specialist and synthesized against their evidence IDs. Investigations are serialized per platform process to avoid overloading the shared local model.

Tools return source, operation, timestamp, duration, success flag, redacted data and a unique evidence ID. Versioned profiles define the role, mandatory checks, allowed operations and interpretation rules. The model can request up to three rounds of bounded tool calls. A specialist has a 150-second model budget; synthesis has a 360-second request budget, and the whole investigation has a 20-minute deadline. Model context is bounded, deduplicated and explicitly marked when excerpts are truncated. Every hypothesis must reference existing observations. Invalid output triggers a limited repair attempt; failure retains the evidence and marks the investigation partial. A valid evidence ID verifies provenance, not correctness of interpretation.

Knowledge-source agents search live API data without a vector database. Configure context_sources separately from runtime dependencies to consult logs, metrics, runbooks and history. The shop bootstrap installs these links. Sources return evidence, never their credentials, to specialists. A production coordinator would need better source selection, retrieval and evaluation.
